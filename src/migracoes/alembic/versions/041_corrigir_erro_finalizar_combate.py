"""Corrigir erro na função finalizar_combate - status_combate constraint
...
def upgrade():
    # Corrigir a função finalizar_combate que está causando erro de constraint
    op.execute("""
        ...
        BEGIN
            ...
            IF p_vencedor = 'jogador' THEN
                ...
                FOR loot_drop IN 
                    SELECT ii.item, ii.quantidade, ii.raridade, ii.drop_chance
                    FROM Inventario_IA ii
                    WHERE ii.id_mob = v_inimigo_id
                LOOP
                    -- Calcular chance de drop baseada na raridade
                    drop_chance := CASE loot_drop.raridade
                        WHEN 'Comum' THEN 0.7
                        WHEN 'Raro' THEN 0.3
                        WHEN 'Épico' THEN 0.1
                        WHEN 'Lendário' THEN 0.05
                        ELSE COALESCE(loot_drop.drop_chance, 0.5)
                    END;
                    ...
                    IF chance_roll <= drop_chance THEN
                        ...
                        SELECT v_jogador_id, i.id_item, loot_drop.quantidade
                        FROM Item i 
                        WHERE i.nome = loot_drop.item
                        ON CONFLICT (Id_PlayerIn, id_item) 
                        DO UPDATE SET quantidade = Inventario_Item.quantidade + EXCLUDED.quantidade;
                        
                        v_loot_texto := v_loot_texto || loot_drop.item || ' x' || loot_drop.quantidade || ', ';
                    END IF;
                END LOOP;
            ...
            ) ON CONFLICT (id_combate) DO NOTHING;
    ...
    # Também vamos limpar qualquer combate com status inválido
    op.execute("""
        -- Corrigir combates com status inválido
        UPDATE Combate 
        SET status_combate = 'finalizado' 
        WHERE status_combate NOT IN ('ativo', 'finalizado', 'fugiu', 'vitoria', 'derrota');
    """)
