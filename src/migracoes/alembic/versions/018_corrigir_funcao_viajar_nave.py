"""Corrigir função viajar_para_planeta - problema com nome_nave
...
def upgrade():
    # Corrigir a função viajar_para_planeta para usar a relação correta com Nave
    op.execute("""
        -- Remover função existente
        DROP FUNCTION IF EXISTS viajar_para_planeta(INT, VARCHAR(20));
        
        -- Recriar função com relação correta para Nave
        ...
        BEGIN
            ...
            -- Verificar se o jogador tem alguma nave
            SELECT COUNT(*) INTO nave_count
            FROM Nave WHERE Id_Player = jogador_id;
            
            IF nave_count = 0 THEN
                RETURN 'Erro: Voce precisa de uma nave para viajar entre planetas';
            END IF;
            
            -- Obter a velocidade da nave mais rápida do jogador
            SELECT MAX(velocidade) INTO velocidade_nave
            FROM Nave WHERE Id_Player = jogador_id;
            ...
            IF velocidade_nave < velocidade_minima THEN
                RETURN 'Erro: Sua nave e muito lenta para viajar para ' || planeta_destino || 
                       ...
                       ', sua nave mais rapida tem: ' || velocidade_nave;
            END IF;
...
def downgrade():
    # Reverter para a versão anterior (com problema de nome_nave)
    op.execute("""
        -- Remover função corrigida
        DROP FUNCTION IF EXISTS viajar_para_planeta(INT, VARCHAR(20));
        
        -- Recriar função com problema de nome_nave
        CREATE OR REPLACE FUNCTION viajar_para_planeta(jogador_id INT, planeta_destino VARCHAR(20))
        RETURNS TEXT AS $$
        DECLARE
        ...
        BEGIN
            ...
            -- Obter dados do jogador (PROBLEMA: nome_nave não existe)
            ...
            FROM Nave WHERE nome_nave = nave_jogador;  -- PROBLEMA: nome_nave não existe
